...
class Database:
    ...
    async def set_prefix(self, id, prefix):
        if prefix is None:
            await self.col.update_one({'id': int(id)}, {'$unset': {'file_prefix': ""}})
            return
        await self.col.update_one({'id': int(id)}, {'$set': {'file_prefix': prefix}})
    async def get_prefix(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('file_prefix', None) if user else None
    async def set_suffix(self, id, suffix):
        if suffix is None:
            await self.col.update_one({'id': int(id)}, {'$unset': {'file_suffix': ""}})
            return
        await self.col.update_one({'id': int(id)}, {'$set': {'file_suffix': suffix}})
    async def get_suffix(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('file_suffix', None) if user else None
    # Metadata Support (per-field, stored as a sub-document)
    async def set_metadata_field(self, id, field, value):
        if value is None:
            await self.col.update_one({'id': int(id)}, {'$unset': {f'metadata.{field}': ""}}, upsert=True)
            return
        await self.col.update_one({'id': int(id)}, {'$set': {f'metadata.{field}': value}}, upsert=True)
    ...
    async def get_sleep(self, id):
        user = await self.col.find_one({'id': int(id)})
        return user.get('sleep_values', None) if user else None
